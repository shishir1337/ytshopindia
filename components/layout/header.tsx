  ...
  return (
    <>
      ...
      <>
        ...
        <div
          className={`fixed top-0 right-0 z-50 h-full w-80 max-w-[85vw] bg-background border-l border-l-primary/20 shadow-2xl transition-transform duration-300 ease-in-out lg:hidden ${
              ...
              <nav className="flex flex-col space-y-1">
                {navItems.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="text-base font-medium transition-colors hover:text-primary hover:bg-accent text-foreground py-3 px-4 rounded-md"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    {item.label}
                  </Link>
